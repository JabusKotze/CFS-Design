...
/*
 * lipped z section properties
 */
public class LippedZSection {
	private String name;
	private double height, width, coatedThickness, lipLength, mass, area, iXX, sectionModulusX, polarRX, iYY, sectionModulusY, polarRY,polarRV, j ;
	private GeneralConstants gC = new GeneralConstants();  // Refers to the the general constants database
	
	
	public LippedZSection(String name, double height, double width, double coatedThickness, double lipLength, double mass, double area, double iXX, double sectionModulusX, double polarRX, double iYY, double sectionModulusY, double polarRY,double polarRV, double j ){
		this.name =name;
		this.height =height;
		this.width =width;
	...
	public double getSectionModulusX(){
		if(sectionModulusX == 0.0){
			...
			return sectionModulusX*1000.0;
		}
	}
	
	public double getSectionModulusY(){
		if(sectionModulusY == 0.0){
			return calcZey();
		}else{
			return sectionModulusY*1000.0;
		}
	}
	...
	public double getWarping(){		//from appendic E: Figure E1
		double warp;
		...
		double term1 = Math.pow(w, 2)*t;
		double term2 = Math.pow(h, 2)*(Math.pow(w, 2)+ 2*w*h+ 4*w*l +6*h*l);
		double term3 = 4*Math.pow(l, 2)*(3*w*h+3*Math.pow(h, 2)+4*w*l+2*h*l+Math.pow(l, 2));
		double term4 = 12*(2*w+h+2*l);
		
		warp = term1*(term2+term3)/term4;
		return warp;
	}
	
	public double getPolarRv(){
		return polarRV;
	}
	...
	public double calcIYY(){
		...
		double r = getangleRadius();
		double iYY;
		double flange = t*Math.pow(width - 2*r, 3)/12. + t*(width - 2.*r)*Math.pow(width/2.,2);
		double lip = (lipLength - r)*Math.pow(t,3)/12. + t*(lipLength - r)*Math.pow(width,2);
		double web = (height-2*r)*Math.pow(t, 3)/12.;
		double cornerA1 = Math.PI*Math.pow(r+t/2., 4)/16. + Math.PI*(Math.pow(r+t/2., 2)/4)*Math.pow(width-r, 2);
		double cornerA2 = Math.PI*Math.pow(r-t/2., 4)/16. + Math.PI*(Math.pow(r-t/2., 2)/4)*Math.pow(width-r, 2);
		double cornerB1 = Math.PI*Math.pow(r+t/2., 4)/16. + Math.PI*(Math.pow(r+t/2., 2)/4)*Math.pow(r, 2);
		double cornerB2 = Math.PI*Math.pow(r-t/2., 4)/16. + Math.PI*(Math.pow(r-t/2., 2)/4)*Math.pow(r, 2);
	...
	public double calcZey(){ // section modulus about the y-axis
		double zey = getIYY()/(width);
		return zey;
	}
	...
	public void printSectionProperties(){
		...
		System.out.println("The Section Modulus about y-y:               Zey = "+getSectionModulusY()+" mm3");
